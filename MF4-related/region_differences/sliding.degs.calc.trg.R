...
## Load data
load(file = paste0(inputdir, "RGC_combine_NEP-tRG_seu.Rdata"))
## rgc_cbn
...
p <- ggplot(anno_df, aes(x = "pseudotime", y = 1))+
			...
			theme_classic() +
			scale_fill_identity()
pdf(paste0(outputdir, "RGC_slidingDEGs_NEP-tRG_xaxis_lable.pdf"), width = 6, height = 4)
print(p)
dev.off()
...
allres <- lapply(allbins, function(bb) {
	...
	msize <- min(table(rgc_cbn$bin, rgc_cbn$lobe)[bb, ])
	if (msize < 100){
		if (bb != 24){
			cc <- c(bb, bb + 1)
		} else {
...
saveRDS(allres, file = paste0(inputdir, "RGC_slidingDEGs_NEP-tRG.rds"))


##------------------------------------------------------------------------
allbins <- 1:24
allres <- readRDS(file = paste0(inputdir, "RGC_slidingDEGs_NEP-tRG_downsample.rds"))
slim_res <- allres %>%
...
out_df <- t(div_df)
write.table(out_df, file = paste0(outputdir, "RGC_diffs_nDEGs_NEP-tRG_log_v2.txt"), row.names = TRUE, col.names = TRUE, quote = TRUE) 
