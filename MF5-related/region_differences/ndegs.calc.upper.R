...
seu <- readRDS(file = paste0(inputdir, "ExN_all_harmony_filtered_IT.rds"))	
...
seu$pseudotime <- it_ptime[colnames(seu), 1]
...
## Balance the age props for "ExN_up_ADRA2A" & "ExN_up_ACTN2"
set.seed(0)
rm_cells1 <- sample(colnames(seu)[seu$subtype %in% c("ExN_up_ADRA2A") & seu$bin %in% 23:36 & seu$cbnage %in% c("E54", "E62-64", "E77-78")], 13000)
rm_cells2 <- sample(colnames(seu)[seu$subtype %in% c("ExN_up_ADRA2A") & seu$bin %in% 23:36 & seu$cbnage %in% c("E93", "E110") & seu$lobe %in% c("MSC", "TC")], 10000)
rm_cells3 <- sample(colnames(seu)[seu$subtype %in% c("ExN_up_ADRA2A") & seu$bin %in% 37:53], 6000)
nseu <- seu[, setdiff(colnames(seu), unique(c(rm_cells1, rm_cells2, rm_cells3)))]
table(nseu$subtype, nseu$cbnage, nseu$lobe)
nseu <- subset(nseu, lobe != "Insula")


saveRDS(nseu, file = paste0(inputdir, "ExN_sliding-window_IT.rds"))


##------------------------------------------------------------------------
## Get bin information
nseu <- readRDS(file = paste0(inputdir, "ExN_sliding-window_IT.rds"))
...
p <- ggplot(anno_df, aes_string(x = "pseudotime", y = "yy", color = "cluster"))+
			...
			scale_fill_identity() +
			theme_classic()
pdf(paste0(outputdir, "ExN_sliding-window_IT_xaxis_lable.pdf"), width = 6, height = 4)
print(p)
dev.off()
...
saveRDS(allres, file = paste0(inputdir, "ExN_sliding-window_IT_DEGres.rds"))
...
pdf(paste0(outputdir, "ExN_sliding-window_IT_nDEGs_byregion.pdf"), width = 10, height = 10)
print(p)
dev.off()
...
out_df <- t(div_df)
write.table(out_df, file = paste0(inputdir, "ExN_sliding-window_IT_nDEGs_log.txt"), row.names = TRUE, col.names = TRUE, quote = TRUE) 
