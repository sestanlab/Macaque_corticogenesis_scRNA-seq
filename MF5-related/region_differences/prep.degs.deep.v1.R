...
cls_ord <- c("IPC EOMES NEUROG1", "IPC EOMES NHLH1 deep", "ExN deep nascent", "ExN deep KIF26A", "ExN deep NR4A2 GRID2", "ExN deep SYT6")
nseu <- seu[, (seu@meta.data$cbnage %in% c("E62-64", "E77-78") ) & (seu@meta.data$subtype %in% cls_ord)]
...
allres <- lapply(complist, function(xx) {
		...
		res
			...
			do.call(rbind, .)
save(allres, file = paste0(inputdir, "Module_pairwise_genes_deep.Rdata"))




##---------------------------------------------------------------------------------
load(file = paste0(inputdir, "Module_pairwise_genes_deep.Rdata"))
...
save(piei, file = paste0(inputdir, "Module_pairwise_pie.dot_deep.Rdata"))
