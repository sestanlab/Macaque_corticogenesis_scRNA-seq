...
cls_ord <- c("IPC EOMES NEUROG1", "IPC EOMES NHLH1 deep", "ExN SOX5 PALMD", "ExN SOX5 ID2", "ExN SOX5 NR4A2 GRID2", "ExN SOX5 SYT6")
nseu <- seu[, (seu@meta.data$cbnage %in% c("E54", "E62-64")) & (seu@meta.data$subtype %in% cls_ord)]
...
allres <- lapply(complist, function(xx) {
		...
		res
			...
			do.call(rbind, .)
saveRDS(allres, file = paste0("./load_files/", "DEGs_RES_bylobe_deep_E54-64.rds"))
